# Adopted from iTerm2 - originally licensed under GPLv2
...
if [[ -o interactive ]]; then
  if [ "${ITERM_ENABLE_SHELL_INTEGRATION_WITH_TMUX-}""$TERM" != "screen" ] &&
    ...
    # Users can write their own version of this method.
    # It should call iterm2_set_user_var but not produce any other output.
    # e.g., iterm2_set_user_var currentDirectory $PWD
    # Accessible in iTerm2 (in a badge now, elsewhere in the future) as \(user.currentDirectory).
    ...
    #      * (ITERM2_PRECMD_PS1 and PS1 are not messed with,
    #        since PS1 already has our escape sequences,
    #        and ITERM2_PRECMD_PS1 already has PS1's original value)
    #    * zsh displays your prompt
    #    * You start entering a command
    #
    ...
    #
    # Invariants:
    # * ITERM2_SHOULD_DECORATE_PROMPT is 1 during and just after command execution,
    #   and "" while the prompt is shown and until you enter a command and press return.
    # * PS1 does not have our escape sequences during command execution
    # * After the command executes but before a new one begins,
    #   PS1 has escape sequences and ITERM2_PRECMD_PS1 has PS1's original value.
    iterm2_decorate_prompt() {
      # This should be a raw PS1 without iTerm2's stuff.
      # It could be changed during command execution.
